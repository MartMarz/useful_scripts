...
filePath = opts.inputDir+"/DY_v6.root"
...
if opts.variables:
    ...
    opts.variables =    [["Pair_Tau_Pt","pt_2"],
                         ...
                         ["Pair_Mu_Phi", "phi_1"],
                         ["Jet_Pt_nom[0]", "jpt_1"],
                         ["taggedJet_Pt_nom[0]", "bpt_1"],
                         ["Jet_Eta_nom[0]", "jeta_1"],
                         ["Jet_Phi_nom[0]", "jphi_1"],
                         ["nTagsM_nom", "nbtag"],
                         ["taggedJet_Eta_nom[0]", "beta_1"],
                         ["taggedJet_Phi_nom[0]", "bphi_1"]]
...
def drawshifts(ogfile, secfile, outdir, processes, variable, variable_og, procLabel = "", procName = ""):
    ...
    for proc in processes:
        ...
        channel = proc
        cutsCrown = "extraelec_veto<0.5 && extramuon_veto<0.5 && pt_1>=25 && abs(eta_1)<=2.1 && abs(eta_2)<=2.3"# && abs(beta_1)<=2.5 && abs(jeta_1)<=2.5 && njets>=2 && nbtag>=1"# && bpt_1>=30" 
        #cutsCrown = "abs(beta_1)<=2.4 && abs(jeta_1)<=2.4 && njets>=2 && nbtag>=1 && extraelec_veto<0.5 && extramuon_veto<0.5 && pt_2>=30 && pt_1>=24 && abs(eta_1)<=2.1 && abs(eta_2)<=2.3"
        cutsSnape = "nMu==1"# && nJets_nom >=2 && nTagsM_nom >=1"# && taggedJet_Pt_nom[0]>=30"
        print("adding {}".format(nomName))
        if (variable == "Pair_Tau_Pt" or variable == "Pair_Mu_Pt" or variable == "taggedJet_Pt_nom[0]" or variable =="Jet_Pt_nom[0]"):

            tree_comp = secfile.Get("Events")
            tmp_comp = ROOT.TH1D("tmp_comp",variable,100,0,400)
            tree_comp.Draw(variable+">>tmp_comp", cutsSnape)
            
            tree_nom = ogfile.Get("ntuple")
            tmp_nom = ROOT.TH1D("tmp_nom",variable,100,0,400)
            tree_nom.Draw(variable_og+">>tmp_nom",cutsCrown)
        ...
        elif (variable == "Pair_Tau_Phi" or variable == "Pair_Mu_Phi" or variable == "Jet_Phi_nom[0]" or variable=="taggedJet_Phi_nom[0]"):
        ...
        elif (variable == "Pair_Tau_Eta" or variable == "Pair_Mu_Eta" or variable == "Jet_Eta_nom[0]" or variable=="taggedJet_Eta_nom[0]"):
...
#    nom.SetMarkerColor(1)
    ...
    lumi = ROOT.TLatex(0.7, 0.94, '13 TeV'  )
    lumi.SetNDC()
...
#    nom.SetMarkerColor(2)
    ...
    ratioComp.GetYaxis().SetTitle("#frac{Crown}{Snape}")
    ratioComp.GetYaxis().CenterTitle()
    ratioComp.GetYaxis().SetRangeUser(0, 2)
