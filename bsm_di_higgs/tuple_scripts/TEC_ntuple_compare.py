...
if opts.variables:
    ...
    opts.variables = [["Pair_Tau_Pt","Pair_Tau_Pt_corr"],
                      ["Pair_Tau_Eta","Pair_Tau_Eta_corr"], 
                      ["Pair_Tau_Phi","Pair_Tau_Phi_corr"], 
                      ["nTaus","nTaus_corr"]
                    ]
...
def drawshifts(ogfile, secfile, outdir, processes, variable, variable_og, procLabel = "", procName = ""):
    ...
    for proc in processes:
        ...
        channel = proc
        cutsnew = "nTaus_corr >= 1 && nMu ==1 && nJets_corr_nom >= 2 && nTagsM_corr_nom >= 1 && Pair_Charge_Valid_corr == 1 && Pair_dR_Valid_corr == 1" 
        cutsold = cutsnew#"nTaus >= 1 && nMu ==1 && nJets_nom >= 2 && nTagsM_nom >= 1 && Pair_Charge_Valid == 1 && Pair_dR_Valid == 1" 
        print("adding {}".format(nomName))
        if (variable == "Pair_Tau_Pt"):
            variable=variable_og
            
            tree_comp = secfile.Get("Events")
            tmp_comp = ROOT.TH1D("tmp_comp",variable_og,100,0,400)
            tree_comp.Draw(variable_og+">>tmp_comp", cutsnew)
            
            tree_nom = ogfile.Get("Events")
            tmp_nom = ROOT.TH1D("tmp_nom",variable,100,0,400)
            tree_nom.Draw(variable+">>tmp_nom", cutsold)

        elif (variable == "nTaus"):
            variable=variable_og

            tree_comp = secfile.Get("Events")
            tmp_comp = ROOT.TH1D("tmp_comp",variable_og,10,0,10)
            tree_comp.Draw(variable_og+">>tmp_comp", cutsnew)
        ...
        elif (variable == "Pair_Tau_Phi"):
            variable=variable_og

            tree_comp = secfile.Get("Events")
            tmp_comp = ROOT.TH1D("tmp_comp",variable_og,50,-4,4)
            tree_comp.Draw(variable_og+">>tmp_comp", cutsnew)
        ...
        elif (variable == "Pair_Tau_Eta"):
            variable=variable_og

            tree_comp = secfile.Get("Events")
            tmp_comp = ROOT.TH1D("tmp_comp",variable_og,50,-3,3)
            tree_comp.Draw(variable_og+">>tmp_comp", cutsnew)
...
#    nom.SetMarkerColor(2)
    ...
    legend.AddEntry(nom, "old", "l")
    legend.AddEntry(comp, "new", "l")
    legend.Draw("")
    ...
    ratioComp.GetYaxis().SetTitle("#frac{old}{new}")
