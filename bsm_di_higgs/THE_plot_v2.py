...
for wan_mhS in wanted_mhS_2D:
    ...
    plots.append(
        ...
        semimu_found_counter,semiele_found_counter, #32
        had_found_counter,semimu_cut_counter,semiele_cut_counter, #35
        had_cut_counter]) #36
...
for point in plots:
    ...
    for mH in point[1]:
        counter_list=[int(point[0]),mH,event_counter[point[1].index(mH)],found[point[1].index(mH)],b_counter[point[1].index(mH)],
                        tau_counter[point[1].index(mH)],comb_cut_counter[point[1].index(mH)],fulllep_counter[point[1].index(mH)]]
...
for quad in counter_tuple:
    ...
    ax.text(MHposition-0.25, mhposition+0.05, "{}".format(format(b_eff, ".2f")), style='italic',color=dRtcolor)
    ax.text(MHposition-0.25, mhposition-0.35, "{}".format(format(t_eff, ".2f")), style='italic',color=dRbcolor)
    if b_eff <= 0.5 and t_eff > 0.5:
        ax.add_patch(Rectangle((MHposition-0.5, mhposition-0.5), 1, 1, linewidth=1, edgecolor=pTtcolor, facecolor=pTtcolor,alpha=0.25))
    elif b_eff > 0.5 and t_eff <= 0.5:
        ax.add_patch(Rectangle((MHposition-0.5, mhposition-0.5), 1, 1, linewidth=1, edgecolor=pTbcolor, facecolor=pTbcolor,alpha=0.25))
    elif b_eff <= 0.5 and t_eff <= 0.5:
        ax.add_patch(Rectangle((MHposition-0.5, mhposition-0.5), 1, 1, linewidth=1, edgecolor='purple', facecolor='purple',alpha=0.25))
...
ax.text(0.5,len(wanted_mhS_2D)-1, 'tau$_{eff} < 0.5$', style='italic', fontsize=fontsize, color=dRtcolor,
        bbox={'facecolor': pTtcolor, 'alpha': 0.15, 'pad': 10})
ax.text(0.5,len(wanted_mhS_2D)-3, 'b$_{eff} < 0.5$', style='italic', fontsize=fontsize, color=dRbcolor,
        bbox={'facecolor': pTbcolor, 'alpha': 0.15, 'pad': 10})
ax.text(0.5,len(wanted_mhS_2D)-5, '$both$', style='italic', fontsize=fontsize,
        bbox={'facecolor': 'purple', 'alpha': 0.15, 'pad': 10})
...
# plt.show()
# fig.tight_layout()
plt.savefig(check+'/2D_individuell_eff.pdf')
...
for quad in counter_tuple:
    ...
    ax.text(MHposition-0.25, mhposition, "{}".format(format(com_eff, ".2f")), style='italic',color='black')
    if com_eff <= 0.5:
        ax.add_patch(Rectangle((MHposition-0.5, mhposition-0.5), 1, 1, linewidth=1, edgecolor='red', facecolor='red',alpha=0.25))
...
ax.text(0.5,len(wanted_mhS_2D)-1, 'com$_{eff} < 0.5$', style='italic', fontsize=fontsize, color='red',
        bbox={'facecolor': 'red', 'alpha': 0.15, 'pad': 10})
